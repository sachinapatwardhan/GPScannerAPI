...
global.CurrentOffset =
  (('00' + offset).slice(-2) >= 0 ? '+' : '-') +
  ('00' + parseInt(offset / 60).toString()).slice(-2) +
  ':' +
  (offset % 60);
...
//uat
var publicKey =
  'BGt6D-TEeBzzYnS1NoihDiCvUBB9C9m4SQ2hhg_cQxZAlQ8Rdu_kEBin-AK0fLfqyzjO94N5GoGTZof4VpGs_A0';
...
webpush.setVapidDetails(
  'mailto:soham.patel@bugzstudio.com',
  publicKey,
  privateKey
);
...
global.sendSMS = function (obj, callback) {
  ...
  request.get(
    {
      url:
        'https://rest.nexmo.com/sms/json?api_key=' +
        api_key +
        '&api_secret=' +
        api_secret +
        '&text=' +
...
app.get('/WebCash', function (req, res) {
  res.redirect(
    '/WebCash.html?Message=' +
      req.query.Message +
      '&ApiRoutePath=' +
      process.env.ApiRoutePath
  );
});
...
io.sockets.on('connection', function (socket) {
  ...
  socket.on('Command9955', function (objGPSData) {
    ...
    io.sockets.emit(
      objGPSData.DeviceId + 'BikeRoute',
      JSON.stringify(objGPSData)
    );
  ...
  socket.on('DeviceAlarm', function (data) {
    ...
    data.Id = 0;
    io.sockets.emit(
      data.IdUser + 'DeviceNotificationCount',
      JSON.stringify(data)
    );
  });
});
...
app.use(
  ...
  require('./controllers/productAttributeMapping')
);
app.use(
  '/productAttributeValue',
  require('./controllers/productAttributeValue')
);
app.use(
  '/productAttributeCombination',
  require('./controllers/productAttributeCombination')
);
app.use('/orderservice', require('./controllers/orderservice'));
app.use('/journey', require('./controllers/journey'));
app.use(
  '/NotificationSetting',
  require('./controllers/NotificationSetting.js')
);
...
app.use(
  '/assignagentretailer',
  require('./controllers/assignagentretailer.js')
);
