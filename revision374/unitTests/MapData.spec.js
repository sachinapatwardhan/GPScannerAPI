...
describe('/gpsdata', function() {
	...
	describe('/MapData/ExportReport', function() {
		it('should export report', function(done) {
			request(server)
			.get('/MapData/ExportReport')
			.query(qs.stringify({
				idUser: dUser.id,
			...
			.end(function(err, res) {
				...
				expect(res.header['content-disposition']).to.equal('attachment; filename=TrackDetail.xlsx');
				done();
			});
		});
	});

	describe('/MapData/GetFenceByPet', function() {
		it('should get all fences when deviceId matches', function(done) {
			request(server)
			.get('/MapData/GetFenceByPet')
			.query(qs.stringify({
	...
	describe('/MapData/GetAllCanvasData', function() {
		it('should get all canvas data when DeviceId matches', function(done) {
			request(server)
			.get('/MapData/GetAllCanvasData')
			.query(qs.stringify({
				DeviceId: testGpsDevice1.DeviceId
			}))
			.expect(200)
			.end(function(err, res) {
				expect(res.body).to.exist;
				expect(res.body).to.be.an('object').that.has.property('Datetime').that.is.equal(testCanvasData1.Datetime);
				done();
			});
		});
	...
	describe('/MapData/GetAllDrivingData', function() {
		it('should get all driving data when DeviceId matches', function(done) {
			request(server)
			.get('/MapData/GetAllDrivingData')
			.query(qs.stringify({
				DeviceId: testGpsDevice1.DeviceId
			}))
			.expect(200)
			.end(function(err, res) {
				expect(res.body).to.exist;
				expect(res.body).to.be.an('object').that.has.property('Datetime').that.is.equal(testDrivingData1.Datetime);
				done();
			});
		});
	...
	describe('/MapData/DeleteFence', function() {
		it('should delete fence when id matches', function(done) {
			request(server)
			.get('/MapData/DeleteFence')
			.query(qs.stringify({
				id: dFence2.id
			}))
			.expect(200)
			.end(function(err, res) {
				expect(res.body).to.exist;
				expect(res.body.success).to.equal(true);
				expect(res.body.message).to.equal('Fence Deleted Successfully...');
				expect(res.body.data).to.equal(1);
				done();
			});
		});
		
		it('should fail when id does not exist', function(done) {
			request(server)
			.get('/MapData/DeleteFence')
			.query(qs.stringify({
				id: 0
