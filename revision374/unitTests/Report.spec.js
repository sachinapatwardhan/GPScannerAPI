...
describe('/Report', function() {
	...
	describe('/Report/GetAllGPSByTimeZoneDate', function() {
		it('should get all GPS by time zone date', function(done) {
			request(server)
			.get('/Report/GetAllGPSByTimeZoneDate')
			.query(qs.stringify({
				TodayStartDateTime: moment.utc().startOf('month').format(),
				TodayEndDateTime: moment.utc().endOf('month').format(),
			...
			.end(function(err, res) {
				...
				expect(res.body.Totalrecord).to.equal(3);
				done();
			});
		});
	});

	describe('/Report/ExportDetailTripReport', function() {
		it('should export detail trip report', function(done) {
			request(server)
			.get('/Report/ExportDetailTripReport')
			.query(qs.stringify({
			...
			.end(function(err, res) {
				...
				expect(res.header['content-disposition']).to.equal('attachment; filename=DetailTripReport.xlsx');
				done();
			});
		});
	});

	describe('/Report/GetAllFenceInAndOutData', function() {
		it('should get all fence in and out data', function(done) {
			request(server)
			.get('/Report/GetAllFenceInAndOutData')
			.query(qs.stringify({
				StartDate: moment.utc().startOf('month').format(),
				EndDate: moment.utc().endOf('month').format(),
				DeviceId: '\'' + testGpsData1.DeviceId + '\'',
			...
			.end(function(err, res) {
				...
				expect(res.body.Totalrecord).to.equal(1);
				done();
			});
		});
	});

	describe('/Report/ExportFenceReport', function() {
		it('should export fence report', function(done) {
			request(server)
			.get('/Report/ExportFenceReport')
			.query(qs.stringify({
				StartDate: moment.utc().startOf('month').format(),
				EndDate: moment.utc().endOf('month').format(),
				DeviceId: '\'' + testGpsData1.DeviceId + '\'',
			...
			.end(function(err, res) {
				expect(res.text).to.exist;
				expect(res.header['content-type']).to.equal('application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
				expect(res.header['content-disposition']).to.equal('attachment; filename=FenceInOutReport.xlsx');
				done();
			});
		});
	});

	describe('/Report/GetAllEngineData', function() {
		it('should get all engine data', function(done) {
			request(server)
			.get('/Report/GetAllEngineData')
			.query(qs.stringify({
				StartDate: moment.utc().startOf('month').format(),
				EndDate: moment.utc().endOf('month').format(),
				DeviceId: '\'' + testGpsData1.DeviceId + '\'',
	...
	describe('/Report/ExportEngineReport', function() {
		it('should export engine report', function(done) {
			request(server)
			.get('/Report/ExportEngineReport')
			.query(qs.stringify({
				StartDate: moment.utc().startOf('month').format(),
				EndDate: moment.utc().endOf('month').format(),
				DeviceId: '\'' + testGpsData1.DeviceId + '\'',
			...
			.end(function(err, res) {
				expect(res.text).to.exist;
				expect(res.header['content-type']).to.equal('application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
				expect(res.header['content-disposition']).to.equal('attachment; filename=EngineOnOffReport.xlsx');
				done();
			});
		});
	});

	describe('/Report/GetAllVehicleLastPositionByUserIdWebApp', function() {
		it('should get vehicle last position by idUser', function(done) {
			request(server)
			.get('/Report/GetAllVehicleLastPositionByUserIdWebApp')
			.query(qs.stringify({
				TodayStartDateTime: moment.utc().startOf('month').format(),
				TodayEndDateTime: moment.utc().endOf('month').format(),
				DeviceId: '\'' + testGpsData1.DeviceId + '\'',
	...
	describe('/Report/ExportLastPositionDataByUserId', function() {
		it('should export last position data by UserId', function(done) {
			request(server)
			.get('/Report/ExportLastPositionDataByUserId')
			.query(qs.stringify({
				StartDate: moment.utc().startOf('month').format(),
				EndDate: moment.utc().endOf('month').format(),
				UserId: dUser.id,
			...
			.end(function(err, res) {
				...
				expect(res.header['content-disposition']).to.equal('attachment; filename=LastPositionReport.xlsx');
				done();
			});
		});
	});

	describe('/Report/GetAllParkingData', function() {
		it('should get all parking data', function(done) {
			request(server)
			.get('/Report/GetAllParkingData')
			.query(qs.stringify({
				TodayStartDateTime: moment.utc().startOf('month').format(),
				TodayEndDateTime: moment.utc().endOf('month').format(),
				DeviceId: '\'' + testGpsData1.DeviceId + '\'',
	...
	describe('/Report/ExportParkingReport', function() {
		it('should export parking report', function(done) {
			request(server)
			.get('/Report/ExportParkingReport')
			.query(qs.stringify({
				StartDate: moment.utc().startOf('month').format(),
				EndDate: moment.utc().endOf('month').format(),
				DeviceId: '\'' + testGpsData1.DeviceId + '\'',
			...
			.end(function(err, res) {
				expect(res.text).to.exist;
				expect(res.header['content-type']).to.equal('application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
				expect(res.header['content-disposition']).to.equal('attachment; filename=ParkingReport.xlsx');
				done();
			});
		});
	});

	describe('/Report/GetAllDailyStatDate', function() {
		it('should get all daily stat date', function(done) {
			request(server)
			.get('/Report/GetAllDailyStatDate')
			.query(qs.stringify({
				TodayStartDateTime: moment.utc().startOf('month').format(),
				TodayEndDateTime: moment.utc().endOf('month').format(),
				DeviceId: '\'' + testGpsData1.DeviceId + '\'',
	...
	describe('/Report/ExportDailyStatReport', function() {
		it('should export daily stat report', function(done) {
			request(server)
			.get('/Report/ExportDailyStatReport')
			.query(qs.stringify({
				StartDate: moment.utc().startOf('month').format(),
				EndDate: moment.utc().endOf('month').format(),
				DeviceId: '\'' + testGpsData1.DeviceId + '\'',
			...
			.end(function(err, res) {
				expect(res.text).to.exist;
				expect(res.header['content-type']).to.equal('application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
				expect(res.header['content-disposition']).to.equal('attachment; filename=DailyStatReport.xlsx');
				done();
			});
		});
	});
