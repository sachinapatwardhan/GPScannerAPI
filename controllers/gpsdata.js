                                    ...
                                    if (resUpdate) {
                                        ...
                                        if (req.query.flg == true || req.query.flg == 'true') {
                                            ...
                                            GpsDeleteCash.findOrCreate({
                                                ...
                                                if (CashCreate[1]) {
                                                    funAuditLog.CreateAuditLog('Create GpsDeleteCash data', decoded.username, 'Save GpsDeleteCash data DeviceId: (' + CashCreate[0].DeviceId + ')');
                                                    res.json({
                                                        success: true,
...
router.post('/GetAllJourneyRouteForReport', jsonParser, function(req, res) {
    ...
    var MaxGpsDate = u.max(LstJourney, function(o) { return new Date(o.journeyEndTime).getTime(); }).journeyEndTime;
    ...
    connectionreport.query(query, function(err, responseGpsData, fields) {
        var query1 = "select tblalarm.*,tblvehicle.deviceid from tblalarm left join tblvehicle on tblalarm.deviceid = tblvehicle.deviceid Where tblvehicle.iduser=" + objParam.idUser + "  and tblalarm.Date >= '" + unixStartdate + "' and tblalarm.Date <= '" + unixEndDate + "'" + wherecondition1 + ";"

        connectionreport.query(query1, function(alarmerr, responseAlarm, alarmfields) {
            if (!err) {
                var TotalAllDrivingTime = 0;
                var TotalAllParkingTime = 0;
                var TotalAllTime = 0;
                ...
                for (var t = 0; t < LstJourney.length; t++) {
                    var jStart = convertdateformatForUnix(LstJourney[t].journeyStartTime);
                    ...
                    var response = u.filter(responseGpsData, function(item) {
                        if (item.Date >= JourneyStartUnix && item.Date <= JourneyEndUnix) {
                            return item;
                        }
                    })
...
router.get('/ExportAlljourneyReportNew', function(req, res) {
    ...
    var WhereCondition = " ";
    var wherecondition1 = " ";
    var WhereJourney = "WHERE  journey.IsDelete = 0 AND  journey.EndTime IS NOT NULL ";

    if (objParam.DeviceId != null && objParam.DeviceId != undefined && objParam.DeviceId != '') {
        WhereCondition += " Where gps.DeviceId in (" + objParam.DeviceId + ")";
        wherecondition1 = ' and tblalarm.deviceid in (' + objParam.DeviceId + ')';
        WhereJourney += ' AND journey.DeviceId in (' + objParam.DeviceId + ') AND  journey.StartTime >="' + objParam.StartDate + '" AND  journey.StartTime <= "' + objParam.EndDate + '" AND  journey.UserId = ' + objParam.idUser + " ";
    }
    if (objParam.idJourney != '' && objParam.idJourney != null && objParam.idJourney != undefined) {
        WhereJourney += " AND journey.Id =" + objParam.idJourney + " ";
    }
    var QueryJourneyData = "Select journey.*,vehicle.Name from tbljourneyroute As journey  INNER JOIN tblvehicle As vehicle ON vehicle.deviceid = journey.DeviceId " + WhereJourney + " ORDER BY journey.StartTime DESC";
    connectionreport.query(QueryJourneyData, function(err, LstJourney, fields) {
        var MinGpsDate = u.min(LstJourney, function(o) {
            return o.StartTime
        }).StartTime;

        var MaxGpsDate = u.max(LstJourney, function(o) { return o.EndTime }).EndTime;
        ...
        connectionreport.query(query, function(err, responseGpsData, fields) {
            ...
            connectionreport.query(query1, function(alarmerr, responseAlarm, alarmfields) {
                ...
                for (var t = 0; t < LstJourney.length; t++) {
                    if (LstJourney[t].JourneyName != null && LstJourney[t].JourneyName != '') {
                        LstJourney[t].DisplayName = LstJourney[t].JourneyName;
                    } else {
                        var StartTime = momentz.utc(new Date(LstJourney[t].StartTime)).tz(req.query.TimeZone).format('DD-MM-YYYY hh:mm:ss a');
                        var EndTime = momentz.utc(new Date(LstJourney[t].EndTime)).tz(req.query.TimeZone).format('DD-MM-YYYY hh:mm:ss a');
                ...
                for (var i = 0; i < response1.length; i++) {
                    var row = [];
                    row.push(response1[i].JourneyName, response1[i].Name, response1[i].DrivingTime, response1[i].Parkingtime, response1[i].TotalTiming, response1[i].TotalMileage, response1[i].AverageSpeed, response1[i].HighestSpeed, response1[i].OverSpeed);
                    conf.rows.push(row);
        ...
        connectionreport.query(query, function(err, responseGpsData, fields) {
            ...
            connectionreport.query(query1, function(alarmerr, responseAlarm, alarmfields) {
                ...
                for (var i = 0; i < response1.length; i++) {
                    table += '<tr>' +
                        '<td style="border-bottom: 1px dotted #000;font-size:10px;font-family: Segoe UI, Tahoma, Geneva, Verdana, sans-serif; border-right: 1px dotted #000; text-align: center;">' + (i + 1) + '</td>' +
                        '<td style="border-bottom: 1px dotted #000;font-size:10px;font-family: Segoe UI, Tahoma, Geneva, Verdana, sans-serif; border-right: 1px dotted #000;">' + response1[i].JourneyName + '</td>' +
                        '<td style="border-bottom: 1px dotted #000;font-size:10px;font-family: Segoe UI, Tahoma, Geneva, Verdana, sans-serif; border-right: 1px dotted #000;">' + response1[i].Name + '</td>' +
                        '<td style="border-bottom: 1px dotted #000;font-size:10px;font-family: Segoe UI, Tahoma, Geneva, Verdana, sans-serif; border-right: 1px dotted #000; text-align: center;">' + response1[i].DrivingTime + '</td>' +
                        '<td style="border-bottom: 1px dotted #000;font-size:10px;font-family: Segoe UI, Tahoma, Geneva, Verdana, sans-serif; border-right: 1px dotted #000; text-align: center;">' + response1[i].Parkingtime + '</td>' +
                        '<td style="border-bottom: 1px dotted #000;font-size:10px;font-family: Segoe UI, Tahoma, Geneva, Verdana, sans-serif; border-right: 1px dotted #000; text-align: center;">' + response1[i].TotalTiming + '</td>' +
