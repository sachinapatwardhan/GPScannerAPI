(function() {
    ...
    router.get('/getPagedLicenceDevicesByAgentId', function(req, res) {
        var offset = (parseInt(req.query.start) * parseInt(req.query.length));
        var objSearch = req.query.Search;
        var WhereCondition = '';
        if (objSearch != '') {
            WhereCondition = 'and tdar.deviceId like "%' + objSearch + '%" ';
        }

        var query = "SELECT tdar.* FROM tbldeviceagentretailer as tdar Inner join tbllicencemanager tl on tdar.deviceId=tl.DeviceId Where tdar.agentId = " + req.query.agentId + " and tl.IsDeleted= 0 " + WhereCondition + " LIMIT " + parseInt(req.query.length) + " offset " + offset + ";";

        var CountQuery = "SELECT count(*) as TotalRecord FROM tbldeviceagentretailer as tdar Inner join tbllicencemanager tl on tdar.deviceId=tl.DeviceId  Where tdar.agentId = " + req.query.agentId + " and tl.IsDeleted= 0 " + WhereCondition;
        connection.query(query, function(err, lstdata, fields) {
            if (!err) {
                connection.query(CountQuery, function(err, lstCount, fields) {
                    var response1 = new Object();
                    response1.recordsTotal = lstCount[0].TotalRecord;
                    response1.data = lstdata;
                    res.json(response1);
                });
            } else {
                var response1 = new Object();
    ...
    router.get('/GetNotAssignDeviceIdByAgentId', function(req, res) {
        var query = "SELECT tdar.deviceId FROM tbldeviceagentretailer as tdar Inner join tbllicencemanager tl on tdar.deviceId=tl.DeviceId Where tdar.agentId = " + req.query.agentId + " and tl.IsDeleted= 0;";
        connection.query(query, function(err, lstdata, fields) {
            if (!err) {
                var List = [];
                u.filter(lstdata, function(p) {
