...
var ExpiryDateDeviceCheck = schedule.scheduleJob(rule, function () {
    ...
    var query1 = "Select id,Name,iduser,deviceid,renewaldate from tblvehicle tv where Date(renewaldate) = '" + convertdateformat(new Date().setMonth(new Date().getMonth() + 1), 4) + "' and IsDelete=false;";
    ...
    var query2 = "Select id,Name,iduser,deviceid,renewaldate from tblvehicle tv where Date(renewaldate) = '" + convertdateformat(new Date().setDate(new Date().getDate() + 15), 4) + "' and IsDelete=false;";
    ...
    var query3 = "Select id,Name,iduser,deviceid,renewaldate from tblvehicle tv where Date(renewaldate) = '" + convertdateformat(new Date().setDate(new Date().getDate() + 3), 4) + "' and IsDelete = false;";
    ...
    var query4 = "Select id,Name,iduser,deviceid,renewaldate from tblvehicle tv where Date(renewaldate) = '" + convertdateformat(new Date().setDate(new Date().getDate() + 2), 4) + "' and IsDelete = false;";
    ...
    var query5 = "Select id,Name,iduser,deviceid,renewaldate from tblvehicle tv where Date(renewaldate) = '" + convertdateformat(new Date().setDate(new Date().getDate() + 1), 4) + "' and IsDelete = false;";
    ...
    var query6 = "Select id,Name,iduser,deviceid,renewaldate from tblvehicle tv where Date(renewaldate) = '" + convertdateformat(new Date(), 4) + "' and IsDelete = true;";
        ...
        if (i < lstExpiredVehicle.length) {
            ...
            connection.query("SELECT tu.id, tu.username,tu.Notification, ta.AppName, ta.IOSCertificate, ta.IOSKey, ta.AndroidId, ta.AndroidSenderId FROM tbluserinformation as tu inner Join tblappinfo as ta ON ta.id = tu.idApp where tu.id=" + objVehicle.iduser, function (err, objAppInfo, fields) {
                var PushNotificationdata = {
                    title: 'Alert',
                    message: "Your tracking services for the " + VehicleName + " will expire " + EndMessage,
                    soundname: 'Default',
                    otherfields: {
