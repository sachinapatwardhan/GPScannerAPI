...
global.deg_to_lat_long = function (deg, Direction) {
  ...
  var sec =
    parseFloat(Minute.substring(Minute.indexOf('.'), Minute.length)) * 60;
  // console.log("#######################")
  // console.log(sec);
  sec = sec.toFixed(3);
  // console.log(degree);
  ...
  var lat_long = '';
  try {
    lat_long = geolib.useDecimal(
      degree + '° ' + min + "' " + sec + '" ' + Direction
    );
  } catch (ex) {
    // console.log(ex)
...
router.get('/SendPushTest', function (req, res) {
  ...
  connection.query(
    'SELECT tu.id, tu.username, ta.AppName, ta.IOSCertificate, ta.IOSKey, ta.AndroidId, ta.AndroidSenderId FROM tbluserinformation as tu inner Join tblappinfo as ta ON ta.id = tu.idApp where tu.id=' +
      UserId,
...
global.SendPushNotification = function (data, UserId, objAppInfo) {
  ...
  var AlarmCode = '';
  if (
    data.otherfields.AlarmCode != null &&
    data.otherfields.AlarmCode != undefined &&
    data.otherfields.AlarmCode != ''
  ) {
  ...
  CheckNotificationOn(UserId, AlarmCode, function (alarmStatusUserId) {
    if (alarmStatusUserId != '') {
      connection.query(
        'SELECT PushNotificationId,Platform,MessageCount,UserType,udid,iduser from tblpushnotification where iduser in (' +
          alarmStatusUserId +
...
function CheckNotificationOn(UserId, AlarmCode, callback) {
  var UserId = UserId;
  var FinalUserId = '';
  var qryUserNotitifcation =
    'Select id from tbluserinformation where id in (' +
    UserId +
    ') and Notification=true';
  connection.query(
    qryUserNotitifcation,
    function (err, lstUserNotificationOn, fields) {
      var NewUserId = '';
      ...
      if (NewUserId != '') {
        var qry =
          'SELECT tnm.Notification, tnm.AlarmCode, tns.IsNotificationOn,tns.idUser FROM tblnotificationmgmt as tnm INNER JOIN tblnotificationsetting as tns ON tns.idNotification = tnm.id where tns.idUser in (' +
          NewUserId +
          ") and (Notification='" +
          AlarmCode +
          "' or AlarmCode = '" +
        ...
        connection.query(qry, function (err, lstNotification, fields) {
          for (var i = 0; i < lstUserNotificationOn.length; i++) {
            var IsNotificationAllow = u.findWhere(lstNotification, {
              idUser: lstUserNotificationOn[i].id,
            });
            if (IsNotificationAllow != undefined) {
              if (
                IsNotificationAllow.IsNotificationOn == true ||
                IsNotificationAllow.IsNotificationOn == 1
              ) {
                if (FinalUserId != '') {
  ...
  try {
    if (
      ...
      CheckNotificationOn(UserId, AlarmCode, function (alarmStatusUserId) {
        if (alarmStatusUserId != '') {
          connection.query(
            'SELECT * from tblpwa_notification_subscription where iduser in (' +
              alarmStatusUserId +
              ')',
            function (errors, lstPWASubscribers, fields) {
                  ...
                  if (p < lstPWASubscribers.length) {
                    try {
                      ...
                      webpush
                        .sendNotification(
                          pushSubscription,
                          JSON.stringify({ title: title, content: message })
                        )
                        .then(function (resPWA) {
                          SendOneByOne(p + 1);
                          // console.log("========================================================")
                          // console.log(resPWA)
                          // console.log("========================================================")
...
function a2hex(str) {
  ...
  for (var i = 0, l = str.length; i < l; i++) {
    var hex = (
      '00' + Number(str.charCodeAt(i)).toString(16).toUpperCase()
    ).slice(-2);
    arr.push(hex);
  }
...
router.get('/GetCenterByGPS', function (req, res) {
  ...
  connection.query(
    "SELECT Latitude as latitude,Longitude as longitude FROM tblgpsdata where DeviceId='" +
      req.query.DeviceId +
      "' and DATE_FORMAT(Datetime,'%H:%i:%s')>='20:30:00' and DATE_FORMAT(Datetime,'%H:%i:%s')<='22:30:00' and speed=0 and Direction=0  and Datetime>'2018-01-02';",
    function (err, lstData, fields) {
...
router.get('/RequestIMEINumberbyUDID', function (req, res) {
  ...
  IMEINumberMapping.findOne({ where: { UDID: UDID } }).then(function (
    objUserIMEI
  ) {
    if (objUserIMEI != null) {
      ...
      IMEINumber.findOne({ where: { IsUse: false } }).then(function (objIMEI) {
        ...
        IMEINumberMapping.create(obj).then(function (resUserIMEI) {
          objIMEI.updateAttributes({ IsUse: true }).then(function (resIMEI) {
            GPSDevice.findOne({ where: { IMEI: objIMEI.IMEI } }).then(function (
              resDevice
            ) {
              if (resDevice == null) {
                var objDevice = new Object();
                objDevice.IMEI = objIMEI.IMEI;
                objDevice.DeviceId = objIMEI.IMEI.toString().substring(
                  objIMEI.IMEI.toString().length - 14
                );
...
router.get('/RequestIMEINumberForAndroid', function (req, res) {
  IMEINumberMapping.findOne({ where: { UDID: req.query.UDID } }).then(function (
    objUserIMEI
  ) {
    if (objUserIMEI != null) {
      res.json({ IMEI: objUserIMEI.IMEI });
      ...
      IMEINumberMapping.create(obj).then(function (resUserIMEI) {
        if (resUserIMEI != null) {
          GPSDevice.findOne({ where: { IMEI: resUserIMEI.IMEI } }).then(
            function (resDevice) {
              if (resDevice == null) {
                var objDevice = new Object();
                objDevice.IMEI = resUserIMEI.IMEI;
                objDevice.DeviceId = resUserIMEI.IMEI.toString().substring(
                  resUserIMEI.IMEI.toString().length - 14
                );
                objDevice.CreatedDate = new Date();
                objDevice.Type = 'Mob';
                objDevice.CreatedBy = '';
...
router.get('/GenerateIMEI', function (req, res) {
  ...
  function InsertIMEI(i) {
    if (i < 10000) {
      ...
      connection.query(
        'SELECT * from tblimeinumber where IMEI=' + IMEINumber + '',
        function (err, numberrow, fields) {
          if (!err) {
            if (numberrow.length > 0) {
              InsertIMEI(i + 1);
            } else {
              connection.query(
                'Insert INTO tblimeinumber (`IMEI`,`IsUse`) VALUES(' +
                  IMEINumber +
                  ',0)',
...
function isStrongEnough(password) {
  ...
  return (
    password.length >= minLength && n && n.length >= numberMinCount && nc && ntf
  );
  // &&
...
router.get('/Command5000', function (req, res) {
  ...
  connection.query(
    'SELECT * from tblgpsdevice where DeviceId=' + DeviceId,
...
global.Command5001 = function (line, Callback) {
  ...
  try {
    ...
    //tblPetgps Entry
    var query =
      "INSERT INTO tblhandshake (DeviceId,Datetime ) VALUES ('" +
      DeviceId +
      "', '" +
      CurrentDate +
      "');";
    connection.query(query, function (err, rows, fields) {
      connection.query(
        "Update tblvehicle set HandshakDatetime='" +
          CurrentDate +
          "',IsOnline=true where deviceid=" +
          DeviceId,
...
global.Command9955 = function (objConnection, Callback) {
  ...
  client.get(
    DeviceId + 'ProjectIgnitionStatus',
    function (err, ProjectIgnitionStatus) {
      if (!err) {
        if (ProjectIgnitionStatus == 'true') {
          ...
          client.get(
            DeviceId + 'EngineStatus1',
            function (err, strEngineStatus) {
              if (!err) {
                if (
                  strEngineStatus != null &&
                  strEngineStatus != undefined &&
                  strEngineStatus != '' &&
                  strEngineStatus != 'null' &&
                  strEngineStatus != 'undefined'
                ) {
              ...
              if (IsEngineStatusGet == false || IsEngineStatusChange == true) {
                ...
                client.set(
                  DeviceId + 'EngineStatus1',
                  JSON.stringify(objnewEngineStatus),
                ...
                if (
                  ObjMyPinIgnition[DeviceId] != null &&
                  ObjMyPinIgnition[DeviceId] != undefined &&
                  ObjMyPinIgnition[DeviceId] != ''
                ) {
                  if (
                    ObjMyPinIgnition[DeviceId].Ignition !=
                      objConnection.IsEngine ||
                    (ObjMyPinIgnition[DeviceId].Ignition ==
                      objConnection.IsEngine &&
                      ObjMyPinIgnition[DeviceId].Alert == false)
                  ) {
                    ObjMyPinIgnition[DeviceId].Ignition =
                      objConnection.IsEngine;
                ...
                function UpdateIgnitionQuery() {
                  var query =
                    "INSERT INTO tblalarm (Datetime, Date, Latitude,Longitude,GPSPositioning,Speed,Direction,Status,DeviceId,AlarmCode,CreatedDate ) VALUES ('" +
                    objConnection.GPSDateTime +
                    "', '" +
                    objConnection.Date +
                    "', '" +
                    objConnection.Latitude +
                    "', '" +
                    objConnection.Longitude +
                    "', '" +
                    objConnection.Position +
                    "', '" +
                    objConnection.Speed +
                    "', '" +
                    objConnection.Direction +
                    "', '" +
                    objConnection.inputoutputSTatus +
                    "', '" +
                    DeviceId +
                    "','" +
                    AlarmCode +
                  ...
                  connection.query(query, function (err, rows, fields) {
                    connection.query(
                      "SELECT id,Name,iduser,deviceid from tblvehicle where deviceid='" +
                        DeviceId +
                        "' and IsDelete=false",
                      function (err, lstVehicle, fields) {
                        if (!err && lstVehicle.length > 0) {
                          var objVehicle = lstVehicle[0];
                          connection.query(
                            'SELECT * from tblsharedevice where idVehicle=' +
                              objVehicle.id +
                              ' and IsSharedUserNotification=true and IsNotification=true',
                            function (err, lstShareUser, fields) {
                              ...
                              if (!err && lstShareUser.length > 0) {
                                for (var i = 0; i < lstShareUser.length; i++) {
                                  ...
                                  AllUser =
                                    AllUser + ',' + lstShareUser[i].idUser;
                                }
                              }
                              connection.query(
                                'SELECT tu.id,tu.idApp, tu.username,tu.email,tu.Notification, ta.AppName, ta.IOSCertificate, ta.IOSKey, ta.AndroidId, ta.AndroidSenderId FROM tbluserinformation as tu inner Join tblappinfo as ta ON ta.id = tu.idApp where tu.id=' +
                                  objVehicle.iduser,
                                function (err, objAppInfo, fields) {
                                  for (var i = 0; i < lstAllUser.length; i++) {
                                    ...
                                    io.sockets.emit(
                                      lstAllUser[i] + 'DeviceAlarm',
                                      JSON.stringify(objConnection1)
                                    );
                                    ObjMyPinIgnition[DeviceId].Alert = true;
                                    ...
                                    io.sockets.emit(
                                      lstAllUser[i] + 'DeviceNotificationCount',
                                      JSON.stringify(objPushnotificationCount)
                                    );
                                  }

                                  client.get(
                                    DeviceId + 'IgnitionStatus',
                                    function (err, UserIgnitionStatus) {
                                      if (!err) {
                                        if (
                                          UserIgnitionStatus != null &&
                                          UserIgnitionStatus != undefined &&
                                          UserIgnitionStatus != '' &&
                                          UserIgnitionStatus != 'null' &&
                                          UserIgnitionStatus != 'undefined'
                                        ) {
                                          if (UserIgnitionStatus == 'true') {
                                            ...
                                            if (AlarmCode == '08') {
                                              Message =
                                                'Vehicle ' +
                                                objVehicle.Name +
                                                ' Ignition ON alert! Please check!';
                                              soundname = 'Default';
                                            } else if (AlarmCode == '07') {
                                              Message =
                                                'Vehicle ' +
                                                objVehicle.Name +
                                                ' Ignition OFF alert! Please check!';
                                              soundname = 'Default';
                                            }
                                                  ...
                                                  if (
                                                    ...
                                                    if (
                                                      ...
                                                      var body =
                                                        ...
                                                        ',' +
                                                        objConnection.Longitude +
                                                        "' target='_blank'>http://maps.google.com/maps?q=" +
                                                        objConnection.Latitude +
                                                        ',' +
                                                        objConnection.Longitude +
                                                        '</a></p>' +
          ...
          client.get(
            DeviceId + 'VehicleStatus1',
            function (err, strVehicleStatus) {
              if (!err) {
                if (
                  strVehicleStatus != null &&
                  strVehicleStatus != undefined &&
                  strVehicleStatus != '' &&
                  strVehicleStatus != 'null' &&
                  strVehicleStatus != 'undefined'
                ) {
                  ...
                  if (objVehicleStatus.VehicleStatus != VehicleStatus) {
                    ...
                    if (objVehicleStatus.IsLastVehicleStatusIdle == true) {
                      ...
                      LastVehicleStatusIdleunixtime =
                        objVehicleStatus.LastVehicleStatusIdleunixtime;
                    }
                  }
                }
              }
              ...
              if (
                ...
                if (VehicleStatus == 2 && IsVehicleStatusGet == true) {
                  if (IsLastVehicleStatusIdle == false) {
                    ...
                    client.set(
                      DeviceId + 'VehicleStatus1',
                      JSON.stringify(objnewVehicleStatus),
                      function (err, replies) {}
                    );
                    VehicleStatus = LastVehicleStatus;

                    if (
                      ObjMyPinIdle[DeviceId] != null &&
                      ObjMyPinIdle[DeviceId] != undefined &&
                    ...
                    client.get(
                      DeviceId + 'IdleMinute',
                      function (err, repliesMinute) {
                        if (
                          repliesMinute != null &&
                          repliesMinute != undefined &&
                          repliesMinute != '' &&
                          repliesMinute != 'null' &&
                          repliesMinute != 'undefined'
                        ) {
                        ...
                        if (
                          objConnection.Date - LastVehicleStatusIdleunixtime >=
                          minute * 60
                        ) {
                          var objnewVehicleStatus = {
                          ...
                          function UpdateIdleQuery() {
                            var AlarmCode = 84;
                            var query =
                              "INSERT INTO tblalarm (Datetime, Date, Latitude,Longitude,GPSPositioning,Speed,Direction,Status,DeviceId,AlarmCode,CreatedDate,FenceName ) VALUES ('" +
                              objConnection.GPSDateTime +
                              "', '" +
                              objConnection.Date +
                              "', '" +
                                    ...
                                    if (!err && lstVehicle.length > 0) {
                                      ...
                                      connection.query(
                                        ...
                                        function (err, lstShareUser, fields) {
                                          var lstAllUser = [objVehicle.iduser];
                                          var AllUser =
                                            objVehicle.iduser.toString();
                                          if (!err && lstShareUser.length > 0) {
                                            for (
                                              var i = 0;
                                              i < lstShareUser.length;
                                              i++
                                            ) {
                                              lstAllUser.push(
                                                lstShareUser[i].idUser
                                              );
                                              AllUser =
                                                AllUser +
                                                ',' +
                                          ...
                                          connection.query(
                                            ...
                                            function (err, objAppInfo, fields) {
                                              ...
                                              var Message =
                                                'Vehicle ' +
                                                objVehicle.Name +
                                                ' Vehical Idle alert! Please check!';
                                              var soundname = 'Default';
                    ...
                    function (err, objAppInfo, fields) {
                      ...
                      if (AlarmCode == '04') {
                        Message =
                          'Vehicle ' +
                          objVehicle.Name +
                          ' Engine ON alert! Please check!';
                        soundname = 'Default';
                      } else if (AlarmCode == '03') {
                        Message =
                          'Vehicle ' +
                          objVehicle.Name +
                          ' Door Open alert! Please check!';
                        soundname = 'Default';
                      } else if (AlarmCode == '10') {
                        Message =
                          'Vehicle ' +
                          objVehicle.Name +
                          ' Low Bettry alert! Please check!';
                        soundname = 'Default';
                      } else if (AlarmCode == '11') {
                        Message =
                          'Vehicle ' +
                          objVehicle.Name +
                          ' Max Speed alert! Please check!';
                        soundname = 'Default';
                      } else if (AlarmCode == '12') {
                        Message =
                          'Vehicle ' +
                          objVehicle.Name +
                          ' Movement alert! Please check!';
                        soundname = 'Default';
                      } else if (AlarmCode == '30') {
                        Message =
                          'Vehicle ' +
                          objVehicle.Name +
                          ' Vibration alert! Please check!';
                        soundname = 'Default';
                      } else if (AlarmCode == '50') {
                        Message =
                          'Vehicle ' +
                          objVehicle.Name +
                          ' External Power Cut alert! Please check!';
                        soundname = 'sound50';
                      } else if (AlarmCode == '05') {
                        Message =
                          'Vehicle ' +
                          objVehicle.Name +
                          ' Original Triggering alert! Please check!';
                        soundname = 'Default';
                      } else if (AlarmCode == '02') {
                        Message =
                          'Vehicle ' +
                          objVehicle.Name +
                          ' Line Broken alert! Please check!';
                        soundname = 'Default';
                      } else if (AlarmCode == '52') {
                        Message =
                          'Vehicle ' +
                          objVehicle.Name +
                          ' Veer Report alert! Please check!';
                        soundname = 'Default';
                      } else if (AlarmCode == '60') {
                        Message =
                          'Vehicle ' +
                          objVehicle.Name +
                          ' Fuel Driving alert! Please check!';
                        soundname = 'Default';
                      } else if (AlarmCode == '71') {
                        Message =
                          'Vehicle ' +
                          objVehicle.Name +
                          ' Crash alert! Please check!';
                        soundname = 'Default';
                      } else if (AlarmCode == '72') {
                        Message =
                          'Vehicle ' +
                          objVehicle.Name +
                          ' Acceleration alert! Please check!';
                        soundname = 'Default';
                      } else if (AlarmCode == '81') {
                        Message =
                          'Vehicle ' +
                          objVehicle.Name +
                          ' Fuel Loss alert! Please check!';
                        soundname = 'Default';
                      } else {
                        soundname = 'Default';
                      }
                      ...
                      client.get(
                        DeviceId + 'EmailNotificationSend',
                        function (err, UserEmailStatus) {
                          // console.log("Redis Error ============================================================", err, UserEmailStatus)
                          if (!err) {
                            if (
                              ...
                              if (UserEmailStatus == 'true') {
                                ...
                                console.log(body);
                                SystemEmail.findOne({
                                  where: { IdApp: objAppInfo[0].idApp },
                                }).then(function (objSystemEmail) {
                                  var mail = {
                                    from: objSystemEmail.DefaultEmailFrom,
                                    to: Emails,
                                    subject:
...
global.SendEmailNotification = function (objConnection) {
  client.get(
    objConnection.DeviceId + 'EmailNotificationSend',
    function (err, UserEmailStatus) {
      if (!err) {
        if (
          UserEmailStatus != null &&
...
function notifyMe(o) {
  ...
  if (o.AlarmCode == '04') {
    ...
    Message =
      'We would like to inform you that your vehicle ' +
      o.Name +
      ' reached over speed.';
  } else if (o.AlarmCode == '03') {
    title = 'Vehicle ' + o.Name + ' Door open alert';
    Message =
      'We would like to notify you that a Door open alert was detected on your vehicle ' +
      o.Name +
      '.';
  } else if (o.AlarmCode == '10') {
    title = 'Vehicle ' + o.Name + ' Low battery alert';
    Message =
      'We would like to notify you that a Low battery alert was detected on your vehicle ' +
      o.Name +
      '.';
    ...
    Message =
      'We would like to notify you that a Movement alert was detected on your vehicle ' +
      o.Name +
      '.';
    ...
    Message =
      'We would like to notify you that a Vibration alert was detected on your vehicle ' +
      o.Name +
      '.';
  } else if (o.AlarmCode == '50') {
    title = 'Vehicle ' + o.Name + ' External power cut alert';
    Message =
      'We would like to notify you that an External power cut alert was detected on your vehicle ' +
      o.Name +
      '.';
  } else if (o.AlarmCode == '05') {
    title = 'Vehicle ' + o.Name + ' Original triggering alert';
    Message =
      'We would like to notify you that an Original triggering alert was detected on your vehicle ' +
      o.Name +
      '.';
  } else if (o.AlarmCode == '02') {
    title = 'Vehicle ' + o.Name + ' Line broken alert';
    Message =
      'We would like to notify you that a Line broken alert was detected on your vehicle ' +
      o.Name +
      '.';
  } else if (o.AlarmCode == '52') {
    title = 'Vehicle ' + o.Name + ' Veer report alert';
    Message =
      'We would like to notify you that a Veer report alert was detected on your vehicle ' +
      o.Name +
      '.';
  } else if (o.AlarmCode == '60') {
    title = 'Vehicle ' + o.Name + ' Fuel driving alert';
    Message =
      'We would like to notify you that a Fuel driving alert was detected on your vehicle ' +
      o.Name +
      '.';
  } else if (o.AlarmCode == '71') {
    title = 'Vehicle ' + o.Name + ' Crash alert';
    Message =
      'We would like to notify you that a Crash alert was detected on your vehicle ' +
      o.Name +
      '.';
  } else if (o.AlarmCode == '72') {
    title = 'Vehicle ' + o.Name + ' Acceleration alert';
    Message =
      'We would like to notify you that an Acceleration alert was detected on your vehicle ' +
      o.Name +
      '.';
  } else if (o.AlarmCode == '81') {
    title = 'Vehicle ' + o.Name + ' Fuel theft  alert';
    Message =
      'We would like to notify you that a Fuel theft alert was detected on your vehicle ' +
      o.Name +
      '.';
  } else if (o.AlarmCode == '04') {
    title = 'Vehicle ' + o.Name + ' Engine ON alert';
    Message =
      'We would like to notify you that an Engine ON alert was detected on your vehicle ' +
      o.Name +
      '.';
  } else if (o.AlarmCode == '08') {
    title = 'Vehicle ' + o.Name + ' Ignition alert';
    Message =
      'We would like to notify you that an Ignition ON alert was detected on your vehicle ' +
      o.Name +
      '.';
  } else if (o.AlarmCode == '07') {
    title = 'Vehicle ' + o.Name + ' Ignition  alert';
    Message =
      'We would like to notify you that an Ignition Off alert was detected on your vehicle ' +
      o.Name +
      '.';
  } else if (o.AlarmCode == '84') {
    title = 'Vehicle ' + o.Name + ' Idle  alert';
    Message =
      'We would like to notify you that your vehicle ' +
      o.Name +
      ' is Idle from last ' +
      o.Time +
      ' minute.';
...
global.Command9901 = function (objCanbusData, Callback) {
  io.sockets.emit(
    objCanbusData.DeviceId + 'canbusdata',
    JSON.stringify(objCanbusData)
  );
};
...
global.Command9902 = function (objDrivingData, Callback) {
  io.sockets.emit(
    objDrivingData.DeviceId + 'drivingdata',
    JSON.stringify(objDrivingData)
  );
};
...
global.SendSpeedData = function (objdata, Callback) {
  ...
  var Speed = ('00' + decimalToHexString(parseInt(objdata.Speed) / 10)).slice(
    -2
  );
  ...
  client.get(DeviceId + 'SocketConnection', function (err, resSocket) {
    ...
    socketclient.connect(ClintSocketPort, ClintSocketIPAddress, function () {
      ...
      socketclient.setTimeout(10000, function () {
        if (Sendflag == false) {
          ...
          Callback({
            success: false,
            message: 'Device not connected. Try after 5 minute.',
          });
        }
      });
    });
    socketclient.on('data', function (data) {
      ...
      if (Sendflag == false) {
        if (
          line.substring(0, 4) == '2424' &&
          line.substring(22, 26) == '4105'
        ) {
          ...
          if (StatusCode == '01') {
            connection.query(
              'Update tblvehicle set MaxSpeed=' +
                objdata.Speed +
                " where deviceid='" +
                DeviceId +
                "'",
              function (err, rows, fields) {
                ...
                Callback({
                  success: true,
                  message: 'Speed Settings Save Successfully.',
                });
              }
            );
          } else {
            Callback({
              success: false,
              message: 'Speed Settings could not save. Try again later.',
            });
          }
        } else {
          Sendflag = true;
    ...
    socketclient.on('data', function (data) {
      ...
      if (Sendflag == false) {
        if (
          ...
          if (StatusCode == '01') {
            connection.query(
              'Update tblvehicle set Movement=' +
                objdata.Movement +
                " where deviceid='" +
                DeviceId +
                "'",
              function (err, rows, fields) {
                funAuditLog.CreateAuditLog(
                  ...
                  'Change vehicle (DeviceId:' +
                    DeviceId +
                    ') Movement Setting at (time:' +
                    convertdateformat(new Date()) +
                    ').'
                );
                Callback({
                  success: true,
                  message: 'Movement Settings Save Successfully.',
                });
              }
            );
          } else {
            Callback({
              success: false,
              message: 'Movement Settings could not save. Try again later.',
            });
          }
        } else {
          Sendflag = true;
    ...
    socketclient.on('data', function (data) {
      ...
      if (Sendflag == false) {
        if (
          ...
          res.json({
            success: true,
            message: 'Current Location Received Successfully.',
          });
        } else {
          Sendflag = true;
          socketclient.destroy();
          res.json({
            success: false,
            message: 'Current Location could not Received. Try again later.',
          });
        }
      }
    });
...
router.get('/SendCommandToDevice', function (req, res) {
  ...
  function uploader(i) {
    if (i < objDevice.length) {
      ...
      SetACCSetting(objACC, function (data) {
        uploader(i + 1);
      });
    } else {
      res.json({
        success: true,
        message: 'Default value send to device successfully.',
      });
    }
  }
  uploader(0);
...
global.SetGPRSInterval = function (objdata, Callback) {
  var DeviceId = objdata.DeviceId;
  var TimeInterval = (
    '0000' + decimalToHexString(parseInt(objdata.TimeInterval) / 10)
  ).slice(-4);
    ...
    socketclient.on('data', function (data) {
      ...
      if (Sendflag == false) {
        if (
          ...
          if (StatusCode == '01') {
            connection.query(
              'Update tblvehicle set GPRSInterval=' +
                objdata.TimeInterval +
                " where deviceid='" +
                DeviceId +
                "'",
              function (err, rows, fields) {
                funAuditLog.CreateAuditLog(
                  'GPRS Interval Settings',
                  null,
                  'Change vehicle (DeviceId:' +
                    DeviceId +
                    ') GPRS Interval Settings at (time:' +
                    convertdateformat(new Date()) +
                    ').'
                );
                Callback({
                  success: true,
                  message: 'GPRS Interval Settings Save Successfully.',
                });
              }
            );
          } else {
            Callback({
              success: false,
              message:
                'GPRS Interval Settings could not save. Try again later.',
            });
          }
        } else {
          Sendflag = true;
          ...
          Callback({
            success: false,
            message: 'GPRS Interval Settings could not save. Try again later.',
          });
        }
      }
    });
      ...
      if (Sendflag == false) {
        if (
          ...
          if (StatusCode == '01') {
            ...
            res.json({
              success: false,
              message: 'Could Not factory Reset. Try again later.',
            });
          }
        } else {
          Sendflag = true;
      ...
      if (Sendflag == false) {
        if (
          ...
          if (StatusCode == '01') {
            ...
            res.json({
              success: false,
              message: 'Could Not Reboot Device. Try again later.',
            });
          }
        } else {
          Sendflag = true;
...
global.SetSleepMode = function (objdata, Callback) {
  var DeviceId = objdata.DeviceId;
  var SleepMode = (
    '00' + decimalToHexString(parseInt(objdata.SleepMode))
  ).slice(-2);
    ...
    socketclient.on('data', function (data) {
      ...
      if (Sendflag == false) {
        if (
          ...
          if (StatusCode == '01') {
            connection.query(
              'Update tblvehicle set SleepMode=' +
                objdata.SleepMode +
                " where deviceid='" +
                DeviceId +
                "'",
              function (err, rows, fields) {
                funAuditLog.CreateAuditLog(
                  ...
                  'Change vehicle (DeviceId:' +
                    DeviceId +
                    ') Sleep Mode Setting at (time:' +
                    convertdateformat(new Date()) +
                    ').'
                );
                Callback({
                  success: true,
                  message: 'Sleep Mode Save Successfully.',
                });
              }
            );
          } else {
            Callback({
              success: false,
              message: 'Sleep Mode could not save. Try again later.',
            });
          }
        } else {
          Sendflag = true;
...
router.get('/SetOutputControl', function (req, res) {
  ...
  if (req.query.DoorLock != undefined) {
    ...
    funAuditLog.CreateAuditLog(
      ...
      'Change vehicle (DeviceId:' +
        req.query.DeviceId +
        ') User Door Lock Setting at (time:' +
        convertdateformat(new Date()) +
        ').'
    );
  }
  if (req.query.DoorUnlock != undefined) {
    ...
    funAuditLog.CreateAuditLog(
      ...
      'Change vehicle (DeviceId:' +
        req.query.DeviceId +
        ') User Door Unlock Setting at (time:' +
        convertdateformat(new Date()) +
        ').'
    );
  }
  var ABCDE =
    ('00' + decimalToHexString(parseInt(ARelay))).slice(-2) +
    ('00' + decimalToHexString(parseInt(BSiren))).slice(-2) +
    ('00' + decimalToHexString(parseInt(CUserDefined))).slice(-2) +
    ('00' + decimalToHexString(parseInt(DDoorLock))).slice(-2) +
    ('00' + decimalToHexString(parseInt(EDoorUnlock))).slice(-2);
      ...
      if (Sendflag == false) {
        if (
          ...
          if (StatusCode == '01') {
            ...
            res.json({
              success: false,
              message: 'Setting could not save. Try again later.',
            });
          }
        } else {
          Sendflag = true;
      ...
      function (err, rows, fields) {
        ...
        client.get(DeviceId, function (err, response) {
          if (
            ...
            res.json({
              success: true,
              message: 'Arm Settings Save Successfully.',
            });
          } else {
            res.json({
              success: true,
      ...
      if (Sendflag == false) {
        if (
          ...
          if (StatusCode == '01') {
            ...
            Callback({
              success: false,
              message: 'Arm Settings could not save. Try again later.',
            });
          }
        } else {
          Sendflag = true;
    ...
    socketclient.on('data', function (data) {
      ...
      if (Sendflag == false) {
        if (
          ...
          if (StatusCode == '01') {
            connection.query(
              'Update tblvehicle set GPRSStopInterval=' +
                objdata.TimeInterval +
                " where deviceid='" +
                DeviceId +
                "'",
              function (err, rows, fields) {
                funAuditLog.CreateAuditLog(
                  'GPRS Interval Setting',
                  null,
                  'Change vehicle (DeviceId:' +
                    DeviceId +
                    ') GPRS Interval Setting at (time:' +
                    convertdateformat(new Date()) +
                    ').'
                );
                Callback({
                  success: true,
                  message:
                    'GPRS Interval Settings for Stop Car Save Successfully.',
                });
              }
            );
          } else {
            Callback({
              success: false,
              message:
                'GPRS Interval Settings for Stop Car could not save. Try again later.',
            });
          }
        } else {
          Sendflag = true;
    ...
    socketclient.on('data', function (data) {
      ...
      if (Sendflag == false) {
        if (
          ...
          if (StatusCode == '01') {
            connection.query(
              'Update tblvehicle set TimeZone=' +
                req.query.TimeZone +
                " where deviceid='" +
                DeviceId +
                "'",
            ...
            res.json({
              success: false,
              message: 'TimeZone could not save. Try again later.',
            });
          }
        } else {
          Sendflag = true;
...
global.SetOdometerSetting = function (objdata, Callback) {
  ...
  Data =
    '4040' +
    ('0000' + datalength.toString(16)).slice(-4) +
    Data +
    CalculateCRCbyHex(Data) +
    '0D0A';
  var socketclient = new net.Socket();
  var Sendflag = false;
  var ClintSocketPort = SocketPort;
    ...
    socketclient.on('data', function (data) {
      ...
      if (Sendflag == false) {
        if (
          ...
          if (StatusCode == '01') {
            connection.query(
              'Update tblvehicle set OdoMeter=' +
                objdata.odometer +
                " where deviceid='" +
                DeviceId +
                "'",
              function (err, rows, fields) {
                funAuditLog.CreateAuditLog(
                  ...
                  'Change vehicle (DeviceId:' +
                    DeviceId +
                    ') Odometer Setting at (time:' +
                    convertdateformat(new Date()) +
                    ').'
                );
                Callback({
                  success: true,
                  message: 'Odometer settings Save Successfully.',
                });
              }
            );
          } else {
            Callback({
              success: false,
              message: 'Odometer settings could not save. Try again later.',
            });
          }
        } else {
          Sendflag = true;
          ...
          if (StatusCode == '01') {
            connection.query(
              ...
              function (err, rows, fields) {
                ...
                Callback({
                  success: true,
                  message: 'ACC settings Save Successfully.',
                });
              }
            );
          } else {
            Callback({
              success: false,
              message: 'ACC settings could not save. Try again later.',
            });
          }
        } else {
          Sendflag = true;
...
global.SetHeartBeatInterval = function (objdata, Callback) {
  ...
  var Data =
    '4040' +
    ('0000' + packetLength.toString(16)).slice(-4) +
    DeviceId +
    '5119' +
    TimeInterval;
  Data = Data + CalculateCRCbyHex(Data) + '0D0A';
    ...
    socketclient.on('data', function (data) {
      ...
      if (Sendflag == false) {
        if (
          ...
          if (StatusCode == '01') {
            connection.query(
              'Update tblvehicle set HeartbeatInterval=' +
                objdata.TimeInterval +
                " where deviceid='" +
                DeviceId +
                "'",
              function (err, rows, fields) {
                funAuditLog.CreateAuditLog(
                  ...
                  'Change vehicle (DeviceId:' +
                    DeviceId +
                    ') HeartBeat Interval Setting at (time:' +
                    convertdateformat(new Date()) +
                    ').'
                );
                Callback({
                  success: true,
                  message: 'HeartBeat Interval Settings Save Successfully.',
                });
              }
            );
          } else {
            Callback({
              success: false,
              message:
                'HeartBeat Interval Settings could not save. Try again later.',
            });
          }
        } else {
          Sendflag = true;
      ...
      if (Sendflag == false) {
        if (
          ...
          if (StatusCode == '01') {
            res.json({
              success: true,
              message: 'Data Logger Clear Successfully.',
            });
          } else {
            res.json({
              success: false,
              message: 'Data Logger could not Clear. Try again later.',
            });
          }
        } else {
          Sendflag = true;
    ...
    socketclient.on('data', function (data) {
      ...
      if (Sendflag == false) {
        if (
          ...
          res.json({
            success: true,
            message: 'Firmware Version get Successfully.',
            data: objFirmware,
          });
        } else {
          Sendflag = true;
          ...
          res.json({
            success: false,
            message: 'could not get Firmware Version. Try again later.',
          });
        }
      }
    });
    ...
    socketclient.on('data', function (data) {
      ...
      if (Sendflag == false) {
        if (
          ...
          res.json({
            success: true,
            message: 'GPRS Time Interval Retrive Successfully.',
            data: GPRSTimeInterval,
          });
        } else {
          Sendflag = true;
          socketclient.destroy();
          res.json({
            success: false,
            message: 'could not get GPRS Time Interval. Try again later.',
          });
        }
      }
    });
    ...
    socketclient.on('data', function (data) {
      ...
      if (Sendflag == false) {
        if (
          ...
          res.json({
            success: true,
            message: 'Trouble Code Retrive Successfully.',
            data: TroubleCode,
          });
        } else {
          Sendflag = true;
          ...
          res.json({
            success: false,
            message: 'could not get Trouble Code. Try again later.',
          });
        }
      }
    });
      ...
      if (Sendflag == false) {
        if (
          ...
          if (StatusCode == '01') {
            res.json({
              success: true,
              message: 'Trouble Code clear successfully.',
            });
          } else {
            res.json({
              success: false,
              message: 'Can Not Clear Trouble Code. Try again later.',
            });
          }
        } else {
          Sendflag = true;
          ...
          res.json({
            success: false,
            message: 'Can Not Clear Trouble Co. Try again later.',
          });
        }
      }
    });
    ...
    socketclient.on('data', function (data) {
      ...
      if (Sendflag == false) {
        if (
          ...
          res.json({
            success: true,
            message: 'VIN Code Retrive Successfully.',
            data: VINCode,
          });
        } else {
          Sendflag = true;
          ...
          res.json({
            success: false,
            message: 'could not get VIN Code. Try again later.',
          });
        }
      }
    });
    ...
    socketclient.on('data', function (data) {
      ...
      if (Sendflag == false) {
        if (
          ...
          res.json({
            success: true,
            message: 'RFID Tags Retrive Successfully.',
            data: RFIDTags,
          });
        } else {
          Sendflag = true;
          ...
          res.json({
            success: false,
            message: 'could not get RFID Tags. Try again later.',
          });
        }
      }
    });
        ...
        if (
          ...
          if (StatusCode == '01') {
            res.json({
              success: true,
              message:
                'Monitor Voice Successfully. You will receive Call soon.',
            });
          } else {
            res.json({
              success: false,
              message: 'Monitor Voice could not retrive. Try again later.',
            });
          }
        } else {
          Sendflag = true;
