...
router.get('/GetAllWorkingBikeWebApp', jsonParser, function (req, res) {
    ...
    var query = "select t4.id,t4.iduser,t4.Name,t4.deviceid,t4.IsOnline,t4.DeviceType,t4.DeviceCompany,t4.FuelRatio,t4.FuelCapacity,t4.IsFule,t4.ShareId,t4.VehicleType,t4.CreatedDate,t4.IdGroup,t4.IdSharedGroup,t4.ExpiryDate,t4.JourneyFlag,  " +
        "(select count(*) from tblalarm  a where a.DeviceId =t4.deviceid and IsRead=false) as 'NotificationCount' ," +
        "(SELECT COUNT(*) FROM tblserviceenhancementnotification WHERE IsRead=false and idvehicle = t4.id) as 'AlertCount' " +
        "from " +
        "  (select tb.iduser,tb.deviceid,tb.id,tb.Name,tb.DeviceType,tb.FuelRatio,tb.FuelCapacity,tb.IsFule,tb.DeviceCompany,tvt.Type as 'VehicleType',tb.CreatedDate,tsd.id as'ShareId',tsd.JourneyFlag,tb.IsOnline,tsd.IdSharedGroup as 'IdSharedGroup',tb.IdGroup, CONVERT_TZ(tb.renewaldate,'+00:00','" + CurrentOffset + "') as ExpiryDate from tblvehicle tb " +
        "   LEFT join tblsharedevice tsd on tb.id=tsd.idVehicle " +
        "   LEFT JOIN tblvehicletype tvt on tb.idType=tvt.id " +
        "   where (tb.iduser=" + req.query.idUser + " or tsd.iduser=" + req.query.idUser + ") and IsDelete = false " +
        "  ) as t4 group by t4.deviceid;";
...
router.get('/GetAllWorkingBikeWebAppNew', function (req, res) {
    ...
    var query = "select t4.id,t4.iduser,t4.Name,t4.deviceid,t4.IsOnline,t4.DeviceType,t4.DeviceCompany,t4.FuelRatio,t4.FuelCapacity,t4.IsFule,t4.ShareId,t4.JourneyFlag,t4.VehicleType,t4.IsShared,t4.IdGroup,t4.IdSharedGroup,t4.ExpiryDate, " +
        "(select count(*) from tblalarm  a where a.DeviceId =t4.deviceid and IsRead=false) as 'NotificationCount' ," +
        "(SELECT COUNT(*) FROM tblserviceenhancementnotification WHERE IsRead=false and idvehicle = t4.id) as 'AlertCount' " +
        "from " +
        "  (select tb.iduser,tb.deviceid,tb.id,tb.Name,tb.IdGroup,tb.DeviceType,tb.DeviceCompany,tb.FuelRatio,tb.FuelCapacity,tb.IsFule,tb.IsShared as 'IsShared',tsd.IdSharedGroup as 'IdSharedGroup',tvt.Type as 'VehicleType',tsd.id as'ShareId',tsd.JourneyFlag,tb.IsOnline, CONVERT_TZ(tb.renewaldate,'+00:00','" + CurrentOffset + "') as ExpiryDate from tblvehicle tb " +
        "   LEFT join tblsharedevice tsd on tb.id=tsd.idVehicle " +
        "   LEFT JOIN tblvehicletype tvt on tb.idType=tvt.id " +
        "   where (tb.iduser=" + req.query.idUser + " or tsd.iduser=" + req.query.idUser + ") and IsDelete = false " +
        "  ) as t4 group by t4.deviceid;";
