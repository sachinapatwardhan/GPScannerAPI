...
router.get('/GetAllGPSByTimeZoneDate', function (req, res) {
  wherecondition = '';
  if (
    req.query.DeviceId != 'All' &&
    req.query.DeviceId != '-1' &&
    req.query.DeviceId != null
  ) {
    wherecondition =
      " and tblgpsdata2.deviceid in ('" + req.query.DeviceId + "')";
  }
  ...
  var query =
    "select tblgpsdata2.Date,tblgpsdata2.IsPatchEngine as IsEngine,tblgpsdata2.Speed,tblgpsdata2.Direction,tblgpsdata2.Latitude,tblgpsdata2.Longitude,tblvehicle.Name from tblgpsdata2 left join tblvehicle on tblgpsdata2.deviceid = tblvehicle.deviceid Where tblgpsdata2.Date >= '" +
    unixStartdate +
    "' and tblgpsdata2.Date <= '" +
    unixEnddate +
    "'" +
    wherecondition +
    orderby +
    ...
    ';';
  var Count =
    "select count(*) AS Totalrecord from tblgpsdata2 Where tblgpsdata2.Date >= '" +
    unixStartdate +
    "' and tblgpsdata2.Date <= '" +
    unixEnddate +
    "'" +
...
router.get('/ExportDetailTripReport', function (req, res) {
  ...
  var query =
    "select tblgpsdata.Date,tblgpsdata.IsPatchEngine as IsEngine,tblgpsdata.Speed,tblgpsdata.Direction,tblgpsdata.Latitude,tblgpsdata.Longitude,tblvehicle.Name from tblgpsdata left join tblvehicle on tblgpsdata.deviceid = tblvehicle.deviceid Where tblgpsdata.Date >= '" +
    unixStartdate +
    "' and tblgpsdata.Date <= '" +
    unixEnddate +
    "'" +
  ...
  connectionreport.query(query, function (err, response) {
    ...
    for (var i = 0; i < response.length; i++) {
      ...
      if (
        ...
        if (
          response[i].DeviceStatus != null &&
          response[i].DeviceStatus != '' &&
          response[i].DeviceStatus != undefined
        ) {
        ...
        if (
          response[i].Fuleper != null &&
          response[i].Fuleper != '' &&
          response[i].Fuleper != undefined
        ) {
          Fuleper = parseFloat(response[i].Fuleper).toFixed(2);
        }
        if (
          response[i].Fulelett != null &&
          response[i].Fulelett != '' &&
          response[i].Fulelett != undefined
        ) {
          Fulelett = parseFloat(response[i].Fulelett).toFixed(2);
        }

        if (
          response[i].Mileage != null &&
          response[i].Mileage != '' &&
          response[i].Mileage != undefined
        ) {
          Mileage = parseFloat(response[i].Mileage).toFixed(2);
        }
        if (
          response[i].Temp != null &&
          response[i].Temp != '' &&
          response[i].Temp != undefined
        ) {
          Temp = parseFloat(response[i].Temp).toFixed(2);
        }

        if (
          response[i].GPSSignal != null &&
          response[i].GPSSignal != '' &&
          response[i].GPSSignal != undefined
        ) {
        ...
        if (
          response[i].Address != null &&
          response[i].Address != '' &&
          response[i].Address != undefined
        ) {
    ...
    res.setHeader(
      'Content-Disposition',
      'attachment; filename=DetailTripReport.xlsx'
    );
...
router.get('/GetAllFenceInAndOutData', function (req, res) {
  ...
  var query =
    'Select tblalarm.* , tblvehicle.iduser, tblvehicle.Name, tblvehicle.IsOnline from tblalarm left join tblvehicle On tblvehicle.deviceid = tblalarm.DeviceId ' +
    search +
    Orderby +
    ' LIMIT ' +
    req.query.length +
    ...
    ';';
  var count =
    'Select count(*) As Totalrecord from tblalarm left join tblvehicle On tblvehicle.deviceid = tblalarm.DeviceId ' +
    search +
    ';';
  connectionreport.query(query, function (err, response) {
      ...
      function GetData(i) {
        if (i < response.length) {
          ...
          if (
            ...
            if (
              response[i].FenceName != null &&
              response[i].FenceName != '' &&
              response[i].FenceName != undefined
            ) {
          ...
          res.setHeader(
            'Content-Disposition',
            'attachment; filename=FenceInOutReport.xlsx'
          );
          res.end(result, 'binary');
        }
      }
    ...
    if (response != undefined) {
        ...
        if (response[k].IsEngine == 1) {
          ...
          if (k != 0 && response[k].GPSPositioning == 'A') {
            if (parseFloat(response[k].Speed) > 1) {
              ...
              TotalMileage += distance(
                parseFloat(response[StartMilage].Latitude),
                parseFloat(response[StartMilage].Longitude),
                parseFloat(response[k].Latitude),
                parseFloat(response[k].Longitude)
              );
              StartMilage = k;
            } else {
              if (Milageco == 0) {
          ...
          if (COuntEngineOn == 0) {
            ...
            obj.StartTime = momentz
              .utc(new Date(response[k].Date * 1000))
              .tz(req.query.TimeZone)
              .format('DD-MM-YYYY hh:mm:ss a');
          }
          COuntEngineOff = 0;
      ...
      for (var j = 0; j < Array.length; j++) {
        if (j != Array.length - 1) {
          ...
          Array[j].ContinueTime = calcDateDiff(
            Array[j].EndTime,
            Array[j].StartTime
          );
          if (Array[j].Status == 'Engine On') {
            TotalEngineOnTime =
              ...
              calcDateDiffCalInSec(
                moment(Array[j + 1].StartTimeold),
                moment(Array[j].StartTimeold)
              );
          }
        } else {
          Array[j].EndTime1 = new Date(
            response[response.length - 1].Date * 1000
          );
          Array[j].EndTime = momentz
            .utc(new Date(response[response.length - 1].Date * 1000))
            .tz(req.query.TimeZone)
            .format('DD-MM-YYYY hh:mm:ss a');
          ...
          if (Array[j].Status == 'Engine On') {
            TotalEngineOnTime =
              TotalEngineOnTime +
              calcDateDiffCalInSec(
                moment(response[response.length - 1].StartTimeold),
                moment(Array[j].StartTimeold)
              );
          }
        }
  ...
  connectionreport.query(query, function (err, response) {
    ...
    if (response != undefined) {
      ...
      for (var i = 0; i < lstEngine.length; i++) {
        ...
        var DatewiseTravelledDistance = 0;
        if (
          lstEngine[i].Name != null &&
          lstEngine[i].Name != '' &&
          lstEngine[i].Name != undefined
        ) {
        ...
        // }
        if (
          lstEngine[i].Status != null &&
          lstEngine[i].Status != '' &&
          lstEngine[i].Status != undefined
        ) {
          Status = lstEngine[i].Status;
        }
        if (
          lstEngine[i].ContinueTime != null &&
          lstEngine[i].ContinueTime != '' &&
          lstEngine[i].ContinueTime != undefined
        ) {
          ContinueTime = lstEngine[i].ContinueTime;
        }
        if (
          lstEngine[i].StartTime != null &&
          lstEngine[i].StartTime != '' &&
          lstEngine[i].StartTime != undefined
        ) {
          StartTime = lstEngine[i].StartTime;
        }
        if (
          lstEngine[i].Mileage != null &&
          lstEngine[i].Mileage != '' &&
          lstEngine[i].Mileage != undefined
        ) {
          Mileage = parseFloat(lstEngine[i].Mileage).toFixed(2);
        }
        if (
          lstEngine[i].EndTime != null &&
          lstEngine[i].EndTime != '' &&
          lstEngine[i].EndTime != undefined
        ) {
      ...
      res.setHeader(
        'Content-Disposition',
        'attachment; filename=EngineOnOffReport.xlsx'
      );
      res.end(result, 'binary');
    } else {
      var response1 = new Object();
...
router.get(
  '/GetAllVehicleLastPositionByUserIdWebApp',
  jsonParser,
  function (req, res) {
    var WhereCondition = '';
    // var StartDate = req.query.StartDate;
    ...
    var StartDate = req.query.TodayStartDateTime;
    var EndDate = req.query.TodayEndDateTime;
    var ExpiredDevice = req.query.ExpiredDevice;
    ...
    if (
      ExpiredDevice != null &&
      ExpiredDevice != undefined &&
      ExpiredDevice != ''
    ) {
      if (WhereCondition == '') {
        WhereCondition = 'where DeviceId not in (' + ExpiredDevice + ') ';
      } else {
        WhereCondition =
          WhereCondition + 'and DeviceId not in (' + ExpiredDevice + ') ';
      }
    }
    ...
    var query =
      'SELECT  tb.*,tpg.IsPatchEngine as IsEngine,tpg.IsDoor, tpg.Latitude,tpg.Longitude,tpg.Datetime, tpg.Date, tpg.Speed, tpg.Direction' +
      ...
      ' (SELECT DeviceId, Date FROM tblgpsdata ' +
      WhereCondition +
      'ORDER BY Date DESC) d GROUP BY DeviceId) ' +
      ...
      req.query.UserId +
      ') and IsDelete=false ' +
      ' group by tb.deviceid order by Name LIMIT ' +
      req.query.length +
      ' OFFSET ' +
      req.query.start +
      ';';
    var count =
      'SELECT count(*) FROM tblvehicle tb left join tblsharedevice tsd ON tsd.idVehicle = tb.id' +
      ...
      ' OR tsd.idUser=' +
      req.query.UserId +
      ') and IsDelete=false group by tb.deviceid;';
...
router.get('/ExportLastPositionDataByUserId', function (req, res) {
  ...
  if (
    ...
    if (WhereCondition == '') {
      WhereCondition =
        'where Date>="' + unixStartdate + '" and Date<="' + unixEnddate + '" ';
    } else {
      WhereCondition =
        WhereCondition +
        'and Date>="' +
  ...
  connectionreport.query(query, function (err, rows, fields) {
    if (!err) {
      for (var i = 0; i < rows.length; i++) {
        rows[i].Time = momentz
          .utc(new Date(rows[i].Date * 1000))
          .tz(req.query.TimeZone)
          .format('DD-MM-YYYY hh:mm:ss a');
      }
      var ListPostionData = rows;
      ListPostionData = u.sortBy(ListPostionData, function (num) {
        return new Date(num.Date);
      });
  ...
  function ExportData(ListPostionData) {
    function AddList(i) {
      if (i < ListPostionData.length) {
        if (
          ListPostionData[i].Latitude != undefined &&
          ListPostionData[i].Latitude != null &&
          ListPostionData[i].Latitude != '' &&
          ListPostionData[i].Longitude != undefined &&
          ListPostionData[i].Longitude != null &&
          ListPostionData[i].Longitude != ''
        ) {
          // geocoder.reverse({ lat: ListPostionData[i].Latitude, lon: ListPostionData[i].Longitude }, function(err, res) {
          if (
            ListPostionData[i].Name != undefined &&
            ListPostionData[i].Name != null &&
            ListPostionData[i].Name != ''
          ) {
          ...
          if (
            ListPostionData[i].deviceid != undefined &&
            ListPostionData[i].deviceid != null &&
            ListPostionData[i].deviceid != ''
          ) {
          ...
          if (
            ListPostionData[i].Time != undefined &&
            ListPostionData[i].Time != null &&
            ListPostionData[i].Time != ''
          ) {
          ...
          if (
            ListPostionData[i].IsOnline != undefined &&
            ListPostionData[i].IsOnline != null &&
            ListPostionData[i].IsOnline != ''
          ) {
          ...
          if (
            ListPostionData[i].IsEngine != undefined &&
            ListPostionData[i].IsEngine != null &&
            ListPostionData[i].IsEngine != ''
          ) {
          ...
          if (
            ListPostionData[i].IsDoor != undefined &&
            ListPostionData[i].IsDoor != null &&
            ListPostionData[i].IsDoor != ''
          ) {
          ...
          if (
            ListPostionData[i].Speed != undefined &&
            ListPostionData[i].Speed != null &&
            ListPostionData[i].Speed != ''
          ) {
          ...
          if (
            ListPostionData[i].Direction != undefined &&
            ListPostionData[i].Direction != null &&
            ListPostionData[i].Direction != ''
          ) {
          ...
          if (
            ListPostionData[i].Datetime != undefined &&
            ListPostionData[i].Datetime != null &&
            ListPostionData[i].Datetime != ''
          ) {
        ...
        res.setHeader(
          'Content-Disposition',
          'attachment; filename=' + 'LastPositionReport.xlsx'
        );
        res.end(result, 'binary');
      }
    }
              ...
              if (IsParking == 1) {
                ...
                TotalParkingtime =
                  ...
                  calcDateDiffCalInSec(
                    ...
                    moment(lstGroup[i].data[ParkingStartPosition].Date)
                  );
                obj.ParkingTime = calhrminsecfromsec(
                  calcDateDiffCalInSec(
                    moment(lstGroup[i].data[k].Date),
                    moment(lstGroup[i].data[ParkingStartPosition].Date)
                  )
            ...
            if (lstGroup[i].data[k].IsEngine == 1) {
              if (IsParking == 1) {
                ...
                obj.EndTime = momentz
                  ...
                  .format('DD-MM-YYYY hh:mm:ss a');
                obj.EndTime1 = lstGroup[i].data[k].Date;
                TotalParkingtime = calcDateDiffCalInSec(
                  moment(lstGroup[i].data[k].Date),
                  moment(lstGroup[i].data[ParkingStartPosition].Date)
                );
                obj.ParkingTime = calhrminsecfromsec(
  ...
  connectionreport.query(query, function (err, response, fields) {
    ...
    function GetData(i) {
      if (i < Array.length) {
        ...
        if (
          Array[i].Latitude != undefined &&
          Array[i].Latitude != null &&
          Array[i].Latitude != '' &&
          Array[i].Longitude != undefined &&
          Array[i].Longitude != null &&
          response[i].Longitude != ''
        ) {
          // geocoder.reverse({ lat: Array[i].Latitude, lon: Array[i].Longitude }, function(err, res) {
          if (
            Array[i].StartTime != null &&
            Array[i].StartTime != '' &&
            Array[i].StartTime != undefined
          ) {
          ...
          if (
            Array[i].ParkingTime != null &&
            Array[i].ParkingTime != '' &&
            Array[i].ParkingTime != undefined
          ) {
        ...
        res.setHeader(
          'Content-Disposition',
          'attachment; filename=ParkingReport.xlsx'
        );
        res.end(result, 'binary');
      }
    }
    ...
    function GetData(i) {
      ...
      if (i < Array.length) {
        ...
        if (
          ...
          Commonfunction.GetAddressLatLong(
            Array[i].Latitude,
            Array[i].Longitude,
            function (resAddress) {
              if (
...
router.get('/GetAllDailyStatDateOld', function (req, res) {
  ...
  var query =
    'select tblgpsdata.*,tblvehicle.Name,tblvehicle.deviceid from tblgpsdata left join tblvehicle on tblgpsdata.deviceid = tblvehicle.deviceid Where tblvehicle.iduser=' +
    req.query.idUser +
    "  and tblgpsdata.Date >= '" +
    unixStartdate +
    "' and tblgpsdata.Date <= '" +
        ...
        if (response.length > 0) {
          var groups = u.groupBy(response, function (value) {
            if (req.query.TimeZone) {
              ...
              return (
                value.DeviceId +
                '#' +
                momentz.utc(value.Date * 1000).format('DD-MM-YYYY')
              );
            }
          });
          ...
          for (var i = 0; i < lstGroup.length; i++) {
            ...
            if (lstGroup[i].data.length > 0) {
              ...
              obj.Date = momentz
                .utc(new Date(lstGroup[i].data[0].Date * 1000))
                .tz(req.query.TimeZone)
                .format('DD-MM-YYYY');
              ...
              if (alarmresponse.length > 0) {
                for (var h = 0; h < alarmresponse.length; h++) {
                  ...
                  var alarmDate = momentz
                    ...
                    .format('DD-MM-YYYY');
                  if (
                    alarmresponse[h].deviceid == obj.DeviceId &&
                    alarmDate == obj.Date
                  ) {
                ...
                if (lstGroup[i].data[k].IsEngine == 1) {
                  if (COuntEngineOff != 0) {
                    if (
                      ...
                      minutes =
                        minutes + timeDifference(StartDate, EndDate, 'm');
                      seconds =
                        seconds + timeDifference(StartDate, EndDate, 's');
                      if (seconds > 60) {
                        seconds = seconds - 60;
                        minutes = minutes + 1;
                      }
                  ...
                  if (CountDriving != 0 && lstGroup[i].data[k].Speed < 1) {
                    if (DrivingEndTime != '') {
                      Drivinghours =
                        Drivinghours +
                        timeDifference(DrivingStartTime, DrivingEndTime, 'h');
                      Drivingminutes =
                        Drivingminutes +
                        timeDifference(DrivingStartTime, DrivingEndTime, 'm');
                      Drivingseconds =
                        Drivingseconds +
                        timeDifference(DrivingStartTime, DrivingEndTime, 's');
                  ...
                  if (COuntEngineON != 0) {
                    if (
                      EngineOnEndDate == null ||
                      EngineOnEndDate == undefined ||
                      EngineOnEndDate == ''
                    ) {
                    } else {
                      EngineOnhours =
                        EngineOnhours +
                        timeDifference(EngineOnStartDate, EngineOnEndDate, 'h');
                      EngineOnminutes =
                        EngineOnminutes +
                        timeDifference(EngineOnStartDate, EngineOnEndDate, 'm');
                      EngineOnseconds =
                        EngineOnseconds +
                        timeDifference(EngineOnStartDate, EngineOnEndDate, 's');
...
router.get('/GetAllDailyStatDate', function (req, res) {
  ...
  connectionreport.query(query, function (err, response, fields) {
    var query1 =
      "select tblalarm.*,tblvehicle.deviceid from tblalarm left join tblvehicle on tblalarm.deviceid = tblvehicle.deviceid Where  tblalarm.Date >= '" +
      unixStartdate +
      "' and tblalarm.Date <= '" +
      unixEnddate +
      "'" +
            ...
            if (lstGroup[i].data.length > 0) {
              ...
              for (var k = 0; k < lstGroup[i].data.length; k++) {
                ...
                if (lstGroup[i].data[k].IsEngine == true) {
                  if (
                    lstGroup[i].data[k].GPSPositioning == 'A' &&
                    parseFloat(lstGroup[i].data[k].Speed) > 1
                  ) {
                    if (
                  ...
                  if (IsEngineOn == 1) {
                    ...
                    TotalEngineOnTime =
                      ...
                      calcDateDiffCalInSec(
                        moment(lstGroup[i].data[k].Date),
                        moment(lstGroup[i].data[EngineOnStartPosition].Date)
                      );
                  }
        ...
        if (response.length > 0) {
          ...
          function GetData(i) {
            if (i < Array.length) {
              ...
              if (
                Array[i].InvalidLocation != null &&
                Array[i].InvalidLocation != '' &&
                Array[i].InvalidLocation != undefined
              ) {
              ...
              if (
                Array[i].AverageSpeed != null &&
                Array[i].AverageSpeed != '' &&
                Array[i].AverageSpeed != undefined
              ) {
                AverageSpeed = parseFloat(Array[i].AverageSpeed).toFixed(2);
              }
              if (
                Array[i].HighestSpeed != null &&
                Array[i].HighestSpeed != '' &&
                Array[i].HighestSpeed != undefined
              ) {
              ...
              if (
                Array[i].AlarmNumber != null &&
                Array[i].AlarmNumber != '' &&
                Array[i].AlarmNumber != undefined
              ) {
                AlarmNumber = Array[i].AlarmNumber;
              }

              if (
                Array[i].EnginOnTime != null &&
                Array[i].EnginOnTime != '' &&
                Array[i].EnginOnTime != undefined
              ) {
                EnginOnTime = Array[i].EnginOnTime;
              }

              if (
                Array[i].DoorOpenNumber != null &&
                Array[i].DoorOpenNumber != '' &&
                Array[i].DoorOpenNumber != undefined
              ) {
                DoorOpenNumber = Array[i].DoorOpenNumber;
              }

              if (
                Array[i].DoorOpenTime != null &&
                Array[i].DoorOpenTime != '' &&
                Array[i].DoorOpenTime != undefined
              ) {
                DoorOpenTime = Array[i].DoorOpenTime;
              }

              if (
                Array[i].ShockNumber != null &&
                Array[i].ShockNumber != '' &&
                Array[i].ShockNumber != undefined
              ) {
                ShockNumber = Array[i].ShockNumber;
              }

              if (
                Array[i].ShockTime != null &&
                Array[i].ShockTime != '' &&
                Array[i].ShockTime != undefined
              ) {
                ShockTime = Array[i].ShockTime;
              }

              if (
                Array[i].EngineOnNumber != null &&
                Array[i].EngineOnNumber != '' &&
                Array[i].EngineOnNumber != undefined
              ) {
              ...
              row.push(
                ...
                OverSpeed,
                AlarmNumber,
                EnginOnTime /*, DoorOpenNumber, DoorOpenTime, ShockNumber, ShockTime, EngineOnNumber*/
              );
              conf.rows.push(row);
              GetData(i + 1);
            } else {
              ...
              res.setHeader(
                'Content-Disposition',
                'attachment; filename=DailyStatReport.xlsx'
              );
              res.end(result, 'binary');
            }
          }
        ...
        if (response.length > 0) {
          ...
          function GetData(i) {
            ...
            if (i < Array.length) {
              ...
              if (
                Array[i].TotalTime != null &&
                Array[i].TotalTime != '' &&
                Array[i].TotalTime != undefined
              ) {
...
router.get('/GetDeviceJourneyRoute', function (req, res) {
  ...
  // }
  if (
    objParam.IdJourneyRoute != null &&
    objParam.IdJourneyRoute != '' &&
    objParam.IdJourneyRoute != undefined
  ) {
